...
async function generateTemplate(loc: string): Promise<void> {
    ...
    if (musicTemplate) setup.push(_default[1]);
    if (panelTemplate) setup.push(_default[2]);
    if (inviteTemplate) setup.push(_default[3]);
    setup.push(
        _default[4].replace(
            '{intents}',
            musicTemplate
    ...
    if (musicTemplate) setup.push(_default[5]);
    if (panelTemplate) setup.push(_default[6]);
    if (inviteTemplate) setup.push(_default[7]);
    ...
    if (musicTemplate || panelTemplate || inviteTemplate) {
        console.log(
            chalk.green(
                `Including ${[musicTemplate && '@akarui/aoi.music', panelTemplate && '@akarui/aoi.panel', inviteTemplate && '@akarui/aoi.invite'].filter(Boolean).join(', ')} in the template.\r\n`
            )
        );
    }
