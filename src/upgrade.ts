...
const packages: string[] = ['aoi.js', '@akarui/aoi.db', '@akarui/aoi.music', '@akarui/aoi.panel', '@akarui/aoi.parser', '@akarui/aoi.invite', 'aoi.parser' /* deprecated */, 'aoi.db' /* deprecated */];
...
async function getDependencies(loc: string): Promise<Dependencies | void> {
    if (!loc) throw new TypeError('Missing location in #getDependencies()');
    ...
    for (const packageName of Object.keys(dependencies)) {
        ...
        if (packageInfo.npm) {
            ...
            if (['aoi.parser', 'aoi.db'].includes(packageName)) {
                deprecated.push(packageName as never);
            }
        }
    }
...
async function packageData(loc: string): Promise<PackageData> {
    ...
    for (let dep of packages) {
        ...
        data.packages.uninstalled = data.packages.uninstalled.filter((dep: { package: string; data: { installed: boolean } }) => dep !== undefined);
    }
