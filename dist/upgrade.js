...
async function updatePackages(loc) {
    ...
    if (data.packages.deprecated.length === 0 &&
...
Happy coding! 🎉`);
    ...
    for (const [i, outdatedPackage] of Object.entries(data.packages.outdated)) {
        ...
        console.log(`• ${packageName} ${chalk.gray(localVersion)} is outdated, ${chalk.gray(npmVersion)} will be installed.`);
    }
    for (const [i, deprecatedPackage] of Object.entries(data.packages.deprecated)) {
        ...
        console.log(`• ${packageName} is deprecated, ${chalk.cyan(`@akarui/${packageName}`)} ${chalk.gray(packageData.npm)} will be installed instead.`);
    }
    console.log(`
${chalk.bgGreen(" install ")}
